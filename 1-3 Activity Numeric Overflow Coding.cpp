...
void test_overflow()
{
    ...
    // Tracks whether overflow occurred
    bool success{};

    std::cout << "\tAdding Numbers Without Overflow (" << +start << ", " << +increment << ", " << steps << ") = ";
    
    // Call add_numbers and store success/failure
    success = add_numbers<T>(start, increment, steps, result);

    // Display result and overflow status
    std::cout << +result << " | overflow: " << std::boolalpha << !success << std::endl;

    std::cout << "\tAdding Numbers With Overflow (" << +start << ", " << +increment << ", " << (steps + 1) << ") = ";
    
    // This call should tigger overflow detection
...
void test_underflow()
{
    ...
    // Stores the result of the subtraction
    T result{};

    // Tracks whether underflow occurred
    bool success{};
